...
float PRateRoll=0.6; float PRatePitch=9; float PRateYaw=0;
float IRateRoll=3.5; float IRatePitch=3.5;float IRateYaw=0;
float DRateRoll=0.03; float DRatePitch=5; float DRateYaw=0;
float Rr_pid_i ,Rp_pid_i ,Ry_pid_i;
float Rr_pid_p; float Rp_pid_p; float Ry_pid_p;
float Rp_pid;float Rr_pid;float Ry_pid;
float ErrorRateRoll; float ErrorRatePitch; float ErrorRateYaw;
float PrevErrorRateRoll, PrevErrorRatePitch, PrevErrorRateYaw;
//============================================================= 
float InputRoll;float InputPitch;float InputYaw;
float w1,w2,w3,w4;
...
void setSpeed(int w1,int w2, int w3 ,int w4){
  ...
  if (w1 < ThrottleIdle) w1 = ThrottleIdle;
  if (w2 < ThrottleIdle) w2 = ThrottleIdle;
  if (w3 < ThrottleIdle) w3 = ThrottleIdle;
  if (w4 < ThrottleIdle) w4 = ThrottleIdle;
